...
import { Loader2, Search, ChevronDown, Send, Save, Info, Upload, FileVideo, X, Settings, Check } from 'lucide-react';
...
const VideoComposer = () => {
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    ...
    if (videoItems.length > 0) {
      const files = videoItems.map(item => item.getAsFile()).filter(Boolean) as File[];
      handleFiles(files);
    }
  };
  ...
  const handlePublish = async () => {
    if (!content.trim() && selectedVideos.length === 0) {
      toast.error('Please add content or videos');
      return;
    }
    ...
    setPublishing(true);
    toast.success('Video post publishing coming soon!');
    setPublishing(false);
  };
  ...
  return (
    <div className="container mx-auto px-6 py-8">
      <h1 className="text-3xl font-bold mb-6">Create video post</h1>
      ...
      <div className="flex gap-6">
            ...
            <div className="p-12 text-center space-y-4">
              <div className="flex justify-center">
                <div className="w-20 h-20 rounded-2xl bg-muted flex items-center justify-center">
                  <FileVideo className="w-10 h-10 text-foreground" strokeWidth={1.5} />
                </div>
              </div>
        ...
        <div className="w-80 space-y-4">
          ...
          <Button 
            variant="outline" 
            className="w-full gap-2 h-12"
            disabled={selectedVideos.length === 0 && !content.trim()}
          >
